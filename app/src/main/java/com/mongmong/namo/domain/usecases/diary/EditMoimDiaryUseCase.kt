...
class EditMoimDiaryUseCase @Inject constructor(
    ...
    suspend fun execute(
        ...
        // 결과를 담을 변수를 초기화
        var diaryResponse: ActionResponse
        val activityResponses = mutableListOf<ActionResponse>()

        // 활동명 공백 여부 체크
        if(activities.any { activity -> activity.title.isEmpty() }) {
            return ActionResponse(isSuccess = false, message = "활동명을 입력해주세요.")
        }
        ...
        } else diaryResponse = ActionResponse(result = "", code = 200, message = "", isSuccess = true)
        ...
        return if (allActivitiesSuccess && diaryResponse.isSuccess) {
            ...
            ActionResponse(message = "Failed to update all activities or diary")
        }
    }
