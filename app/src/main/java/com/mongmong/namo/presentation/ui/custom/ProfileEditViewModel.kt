...
class ProfileEditViewModel @Inject constructor(
    ...
    // 닉네임, 이름 공개 여부
    val isPublicFields = MutableLiveData<Map<String, Boolean>>(
        mapOf(
            "nickname" to false,
            "birthday" to false
        )
    ...
    val highlightFields = MutableLiveData<Map<String, Boolean>>(
        mapOf(
            "nickname" to false,
            "birth" to false,
            "color" to false
    ...
    fun editProfile() {
        viewModelScope.launch {
            _isEditComplete.value = registerUseCase.invoke(
                name = name.value ?: "",
